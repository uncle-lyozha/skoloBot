...
export class AddJokeWizard {
  ...
  async onEnter(@Ctx() ctx: WizardContext) {
    const msg = this.script.addJokeWiz.onEnter;
    await ctx.reply(msg);
    ctx.wizard.next();
  }
  ...
  async onFact(
    ...
    const text = ctx.text;
    try {
      await this.bookRep.createJokeRecord(userName, id, text);
      const msg = this.script.addJokeWiz.onEnd;
      await ctx.reply(msg);
    } catch (err) {
      console.error(err);
    }
